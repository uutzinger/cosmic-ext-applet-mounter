...
- [ ] Document the current host-integration requirements: executable discovery,
...
- [ ] Verify host command argument passing, exit status, standard output,
  ...
  `flatpak-spawn --host`.
- [ ] Determine which configuration belongs inside the Flatpak and which must
...
- [ ] Reject any design that silently uses a different rclone/OneDrive config,
...
- [ ] Record the selected architecture and its security tradeoffs in
